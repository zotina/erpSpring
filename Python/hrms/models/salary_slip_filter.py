...
class SalarySlipFilter:
    ...
    def get_filtered_salary_slips(self, salary_component, reference_amount, comparison_type, yearDebut, yearFin):
        """
        ...
        Args:
            ...
            yearDebut (str): Année de début
            yearFin (str): Année de fin
        Returns:
        ...
        if yearDebut and yearDebut.strip():
            conditions.append("sl.posting_date >= %(yearDebut)s")
            query_params['yearDebut'] = f"{yearDebut}-01"
        
        if yearFin and yearFin.strip():
            conditions.append("sl.posting_date <= %(yearFin)s")
            query_params['yearFin'] = f"{yearFin}-01"
        
        condition_date = ""
        if conditions:
            condition_date = "WHERE " + " AND ".join(conditions)
